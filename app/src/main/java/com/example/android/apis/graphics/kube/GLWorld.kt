 ...
 * `IntBuffer mVertexBuffer`, the color buffer `IntBuffer mColorBuffer` and the index
 * buffer `ShortBuffer mIndexBuffer`. It also contains the method `transformVertex` which
 * will apply an `M4` transformation matrix to a vertex so that the rubic cube can be animated.
 * The `draw` method is called from the `onDrawFrame` callback of `KubeRenderer`
 * whenever it is necessary to draw the latest version of our rubic cube (`generate` has to have
...
class GLWorld {
     ...
     * allocate space for `ShortBuffer mIndexBuffer` and as the `count` argument to the
     ...
     * `GLVertex.put` which adds its color to `IntBuffer mColorBuffer`.
     */
     ...
     * the entire Rubic cube has been generated, each `GLVertex` in our list `mVertexList`
     * is instructed to add its color to `IntBuffer mColorBuffer` (in the same call to
     ...
     * `ArrayList<GLShape> mShapeList` to add its indices to `mIndexBuffer` using its
     * method `putIndices`.
     */
    private var mIndexBuffer: ShortBuffer? = null

    /**
     * Adds its parameter `GLShape shape` to our list `ArrayList<GLShape> mShapeList` and
     * updates `mIndexCount` by adding the number of indices required by the `GLShape` to
     * it.
     *
     * @param shape `GLShape` to add to `ArrayList<GLShape> mShapeList`
     */
    ...
    /**
     * Constructs a `GLVertex vertex` from its parameters, adds it to our list of vertices
     * `ArrayList<GLVertex> mVertexList`, and returns it to the caller.
     *
     * @param x x coordinate of new vertex
     * @param y y coordinate of new vertex
     * @param z z coordinate of new vertex
     * @return the `GLVertex` object we have constructed and added to our list of vertices
     * `ArrayList<GLVertex> mVertexList`
     */
     ...
     * when it draws our Rubic cube: `IntBuffer mColorBuffer`, `IntBuffer mVertexBuffer`
     * and `ShortBuffer mIndexBuffer`. First we direct allocate `ByteBuffer bb` to be
     * the correct number of bytes required for a color buffer (4 bytes per component, and 4 components
     * for each vertex in `ArrayList<GLVertex> mVertexList`), set its byte order to native order
     * and initialize `IntBuffer mColorBuffer` with a view of `bb` as an int buffer.
     * Then we direct allocate `bb` to have the correct number of bytes required for a vertex
     * buffer (4 bytes per coordinate, and 3 coordinates per vertex), set its byte order to native
     * order and initialize `IntBuffer mVertexBuffer` with a view of `bb` as an int buffer.
     ...
     * `ShortBuffer mIndexBuffer` with a view of `bb` as an short buffer.
     *
     *
     * Now we iterate through all the `GLVertex` objects in `ArrayList<GLVertex> mVertexList`
     * fetching the next `GLVertex vertex` and calling its `put` method to add its
     * coordinates to `IntBuffer mVertexBuffer` and its color to `IntBuffer mColorBuffer`.
     *
     *
     * Finally we iterate through all the `GLShape` objects in `ArrayList<GLShape> mShapeList`
     * fetching the next `GLShape shape` and calling its `putIndices` method to add the
     * index values to `ShortBuffer mIndexBuffer` which will divide the `GLShape` into
     * GL_TRIANGLES for `glDrawElements` to draw.
     */
    fun generate() {
    ...
    /**
     * Applies a `M4 transform` to the coordinates of a `GLVertex vertex` and stores the
     * result in our direct allocated vertex buffer `IntBuffer mVertexBuffer` ready for the next
     * call to `draw` (the original coordinates of `GLVertex vertex` remain unchanged).
     * Called from `GLShape.animateTransform` for each `GLVertex` in the `GLShape`,
     * it is called from `Layer.setAngle`, which is called from `Kube.animate`, which is
     * called from `KubeRenderer.onDrawFrame`. We simply call the method `vertex.update`
     * which multiplies its coordinates by the `transform` transform matrix and places the new
     * values in the correct position in our field `IntBuffer mVertexBuffer`.
     *
     * @param vertex    `GLVertex` to apply the `M4 transform` to
