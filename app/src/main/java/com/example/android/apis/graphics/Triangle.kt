...
class Triangle {
     ...
     * (selects counterclockwise polygons as front-facing). We next define our [FloatBuffer] field
     * [mFVertexBuffer] to be our array of vertex data, with 3 coordinates per vertex, using GL_FLOAT
     * as its data type, and a stride of 0. We enable the server-side GL capability GL_TEXTURE_2D
     * (two-dimensional texturing is performed), then define our [FloatBuffer] field [mTexBuffer]
     * to be our array of texture coordinates, with 2 coordinates per point, using GL_FLOAT as its
     * data type, and a stride of 0.
     *
     * Finally we call the [GL10.glDrawElements] method of [gl] to render primitives from array data
     * specifying our [ShortBuffer] field [mIndexBuffer] as our indices array, GL_UNSIGNED_SHORT as
     * its data type, and GL_TRIANGLE_STRIP as the type of primitive to render (Every group of 3
     * adjacent vertices forms a triangle - we have only one triangle).
     *
