...
class PrintHtmlOffScreen : AppCompatActivity() {
    private var mWebView // WebView we load our HTML file into and print
            : WebView? = null
     ...
     * super's implementation of `onCreateOptionsMenu`, then we fetch a `MenuInflater` and
     * use it to inflate our menu resource file R.menu.print_custom_content into `menu`. We
     ...
     * if the ID of `MenuItem item` is the same as our R.id.menu_print and if it is we call
     * our method `print()` and return true to the caller (to indicate that we consumed the
     ...
     * First we create a `WebView` for our field `WebView mWebView`. Then we set the
     * `WebViewClient` of `mWebView` to an anonymous class which calls our `doPrint`
     * method only when the load of the HTML file completes. Finally we instruct `mWebView` to
     * load our HTML file "file:///android_res/raw/motogp_stats.html"
     */
    private fun print() { // Create a WebView and hold on to it as the printing will start when
// load completes and we do not want the WbeView to be garbage collected.
    ...
    /**
     * Causes our field `WebView mWebView` to print its contents. First we get a handle to the
     * system level PRINT_SERVICE for `PrintManager printManager`, then we create an instance
     * of `PrintDocumentAdapter adapter` which "wraps" around the `PrintDocumentAdapter`
     * created by our `WebView mWebView`. The wrapping allows us to add code to the `onFinish`
     * callback to destroy `mWebView`, and set it to null. All other callbacks simply call through
     * to the `PrintDocumentAdapter mWrappedInstance` created by `mWebView`. Finally we
     * pass the wrapper `adapter` to `printManager` to print the HTML in `mWebView`.
     */
    private fun doPrint() { // Get the print manager.
        ...
        val adapter: PrintDocumentAdapter = object : PrintDocumentAdapter() {
             ...
             * `WdbView mWebView`, and set it to null.
             */
