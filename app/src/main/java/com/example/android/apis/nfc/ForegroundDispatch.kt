...
class ForegroundDispatch : AppCompatActivity() {
     ...
     * with the data type "&#42;&#47;&#42;". We use it in our call to the `enableForegroundDispatch`
     * method of `NfcAdapter mAdapter`.
     */
     ...
     * of our call to the `enableForegroundDispatch` method of `NfcAdapter mAdapter`.
     ...
     * We initialize our field `TextView mText` by locating the `TextView` in our layout
     * with ID R.id.text, and set its text to the string "Scan a tag". We initialize our field
     * `NfcAdapter mAdapter` with the default NFC Adapter of our device. We initialize our field
     * `PendingIntent mPendingIntent` with a `PendingIntent` which contains an Intent to
     * launch this activity with the flag FLAG_ACTIVITY_SINGLE_TOP (so the Intent will be delivered
     * to our method `onNewIntent` instead of starting a new instance). The request code of the
     * `PendingIntent` is 0, and the flags are also zero. Then we create `IntentFilter ndef`
     * with the action ACTION_NDEF_DISCOVERED, add the data type "&#42;&#47;&#42;" (mime type for all
     * types of data), and use it as the single entry in the `IntentFilter[]` array we create
     * to initialize `IntentFilter[] mFilters`. We initialize our field `String[][] mTechLists`
     * with an array containing the single entry `NfcF.class.getName()` ("android.nfc.tech.NfcF")
     *
     ...
     * implementation of `onResume`, then if our field `NfcAdapter mAdapter` is not null
     * we call its method `enableForegroundDispatch` to Enable foreground dispatch to this
     * activity as specified in the `PendingIntent mPendingIntent`, filtering based on the
     * contents of `IntentFilter[] mFilters` and also matching the tech lists in our field
     * `String[][] mTechLists`.
     */
     ...
     *
     * We simply we the text of our `TextView mText` to display the string "Discovered tag",
     * concatenated with the value of `mCount` (which we then increment), concatenated with
     * the string "with intent:", concatenated with the string representation of the `Intent intent`
     ...
     * then if our field `NfcAdapter mAdapter` is not null we call its `disableForegroundDispatch`
