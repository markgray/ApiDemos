...
public class ContentBrowserNavActivity extends Activity
    ...
    public static class Content extends ScrollView
         ...
         * SYSTEM_UI_FLAG_IMMERSIVE in {@code newVis}. We initialize {@code boolean changed} to true
         * if {@code newVis} is the same as the last system UI that was requested using {
         *
         * @param visible true makes our navigation elements visible, false makes them invisible.
         * @code setSystemUiVisibility(int)} (is this logic inverted?). If {@code changed} or
         * {@code visible} is true, we initialize {@code Handler h} with a handler associated with
         * the thread running our View, and if the result is not null we remove any scheduled
         * {@code Runnable mNavHider} from the queue. We then call {@code setSystemUiVisibility}
     ...
     * R.layout.content_browser_nav. We initialize our field {@code Content mContent} by finding the
