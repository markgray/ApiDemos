...
class LaunchingPreferences : AppCompatActivity(), View.OnClickListener {
     ...
     * false as the `boolean readAgain` flag so this will only happen once).
     *
     *
     * Next we create `LinearLayout layout`, set its orientation to VERTICAL and set it as our
     * content view. We create `Button launchPreferences`, set its text to the string
     * R.string.launch_preference_activity ("Launch PreferenceActivity"), set its `OnClickListener`
     * to "this" and add it to `LinearLayout layout` using the layout parameters MATCH_PARENT
     * and WRAP_CONTENT for the width and height. Next we initialize our field `TextView mCounterText`
     * with a new instance and add it to `LinearLayout layout` using the layout parameters MATCH_PARENT
     * and WRAP_CONTENT for the width and height.
     *
     *
     * Finally we call our method `updateCounterText` to read the preference stored under the
     * key `AdvancedPreferences.KEY_MY_PREFERENCE` ("my_preference"), then format and display
     * it in `TextView mCounterText`.
     *
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        layout.addView(launchPreferences, LinearLayout.LayoutParams(LinearLayout.LayoutParams.MATCH_PARENT, LinearLayout.LayoutParams.WRAP_CONTENT))
        mCounterText = TextView(this)
        layout.addView(mCounterText, LinearLayout.LayoutParams(LinearLayout.LayoutParams.MATCH_PARENT, LinearLayout.LayoutParams.WRAP_CONTENT))
        updateCounterText()
    }

    /**
     * Called when our "Launch PreferenceActivity" button is clicked. First we create an intent
     * `Intent launchPreferencesIntent` to launch `AdvancedPreferences`, then we use
     * it to start that activity for a result using the request code REQUEST_CODE_PREFERENCES (1).
     * When that activity is finished, the result will be passed to our `onActivityResult`
     * override.
     *
     ...
     * of `onActivityResult`, then if the `requestCode` parameter is REQUEST_CODE_PREFERENCES
     * (the code we started `AdvancedPreferences` with), we call our method `updateCounterText`
     * to read the preference stored under the key `AdvancedPreferences.KEY_MY_PREFERENCE`
     * ("my_preference"), then format and display it in `TextView mCounterText`.
     *
     ...
     * ("my_preference"), then format and display it in `TextView mCounterText`. First we fetch
     * a `SharedPreferences` instance that points to the default file that is used by the
     * preference framework in the given context to initialize `SharedPreferences sharedPref`.
     * We fetch the int stored under the key `AdvancedPreferences.KEY_MY_PREFERENCE` in
     * `sharedPref` to initialize `int counter`. Then we concatenate the string
     * R.string.counter_value_is ("The counter value is") to a space followed by the string value of
     * `counter` and set the text of `TextView mCounterText` to it.
     */
