...
class ShowInputMethodAndSubtypeEnabler : AppCompatActivity() {
     ...
     * VERTICAL. We initialize `Button button` with a new instance, set its text to the string
     * "Show (All IMEs)", set its `OnClickListener` to an anonymous class which calls our method
     * `showInputMethodAndSubtypeEnabler` with null as the input method id, and then add `button`
     * to `layout`.
     *
     *
     * Next we loop over the list of `InputMethodInfo imi` that is returned by our method
     * `getEnabledInputMethodsThatHaveMultipleSubtypes`, initializing `Button button` with
     * a new instance, initializing `String id` with the id of `imi`, setting the text of
     * `button` to the string formed by concatenating the string "Show (" followed by `id`
     * followed by the string ")", set its `OnClickListener` to an anonymous class which calls
     * our method `showInputMethodAndSubtypeEnabler` with `id` as the input method id,
     * and then add `button` to `layout` before looping around for the next `InputMethodInfo`.
     *
     *
     ...
     * we initialize `InputMethodManager imm` with a handle to the system level service
     * INPUT_METHOD_SERVICE, and initialize `List<InputMethodInfo> result` with a new instance.
     * If `imm` is null we return `result`. Otherwise for all `InputMethodInfo imi`
     * in the list of `InputMethodInfo` returned by the `getEnabledInputMethodList` method
     * of `imm`  we add `imi` to `result`. When done we return `result` to the
     * caller.
     *
    ...
    companion object {
         ...
         * input method id (or all methods if our parameter `String inputMethodId` is null or empty).
         * First we initialize `Intent intent` with a new instance whose action is specified to be
         ...
         *
         *
         * If our parameter `String inputMethodId` is not null or empty, we add it as an extra to
