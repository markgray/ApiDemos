...
class PolyToPoly : GraphicsActivity() {
    ...
    private class SampleView(context: Context?) : View(context) {
        ...
        /**
         * `FontMetrics` of the `Paint mPaint` used to center text we draw.
         */
         ...
         * in the center representing the number of points in the `setPolyToPoly` transformation
         * matrix used to warp it. The transformation matrix is created using the `float src[]`
         * and `float dst[]` parameters.
         *
         *
         * First we save the current matrix and clip of `Canvas canvas` onto a private stack.
         * Then we set `Matrix mMatrix` to a matrix such that the points specified in our parameter
         * `float src[]` would map to the points specified by our parameter `float dst[]`.
         * We then pre-concatenate `mMatrix` to the current matrix of `Canvas canvas`.
         *
         *
         * We set the color of `Paint mPaint` to GRAY, set its style to STROKE, and then use it
         * to draw a rectangle, and two lines connecting the nonadjacent vertices to `canvas`.
         *
         *
         * We set the color of `mPaint` to RED, and set its style to FILL. The calculate the
         * center of our rectangle based on the size of the rectangle and the size of the font used
         * by `mPaint` then draw the number of points in our `src` array at the center of
         * the rectangle.
         *
         *
         * Finally we restore the current matrix of the `Canvas canvas` to the state it was in
         * before the `save` call at the beginning of our method.
         *
...
// centering in X is easy... use alignment (and X at midpoint)
            ...
            canvas.drawText((src.size / 2).toString() + "", x, y, mPaint)
            canvas.restore()
        }

        /**
         * We implement this to do our drawing. First we set our entire `Canvas canvas` to the
         * color WHITE. Then we produce four different examples using the method `setPolyToPoly`
         * each wrapped between matching `canvas.save` and `canvas.restore` calls:
         *
         *  *
        ...
        override fun onDraw(canvas: Canvas) {
            ...
            // rotate/skew (3 points)
            doDraw(canvas, floatArrayOf(0f, 0f, 64f, 0f, 0f, 64f), floatArrayOf(0f, 0f, 96f, 0f, 24f, 64f))
            canvas.restore()
            canvas.save()
            canvas.translate(160f, 110f)
            // perspective (4 points)
            doDraw(canvas, floatArrayOf(0f, 0f, 64f, 0f, 64f, 64f, 0f, 64f), floatArrayOf(0f, 0f, 96f, 0f, 64f, 96f, 0f, 64f))
            canvas.restore()
        }

        /**
         * Our constructor. First we call our super's constructor, then we set the stroke width of
         * `Paint mPaint` to 4, its text size to 40, and its text alignment to CENTER. Finally
         * we initialize our field `Paint.FontMetrics mFontMetrics` with a font metrics object
         * filled with the appropriate values given the text attributes of `Paint mPaint`.
         *
         * Parameter: context `Context` to access resources.
         */
        init {
