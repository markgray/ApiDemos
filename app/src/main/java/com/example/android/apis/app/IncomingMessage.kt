...
class IncomingMessage : AppCompatActivity() {
     ...
     * We locate the `Button` R.id.notify_app to initialize `Button button`, then set
     * its `OnClickListener` to an anonymous class which calls the method `showAppNotification()`
     * when the `Button` is clicked. Then we locate the `Button` R.id.notify_interstitial
     * and set its `OnClickListener` to an anonymous class which calls the method
     ...
     * We are called when the `Button` R.id.notify_app ("Show App Notification") is clicked.
     * First we fetch a handle to the system-level service NOTIFICATION_SERVICE and save it in our
     * variable `NotificationManager nm`. We initialize `NotificationChannel chan1` with
     ...
     * We set the notification light color of `chan1` to GREEN, and set its lock screen visibility
     * to VISIBILITY_PRIVATE (shows this notification on all lockscreens, but conceal sensitive or
     * private information on secure lockscreens). We then have `nm` create notification channel
     * `chan1`. We create a fake message to receive consisting of a `CharSequence from`
     * and a random `CharSequence message`. Next we create a `PendingIntent contentIntent`
     * which will be launched when our notification is clicked. It consists of a back stack of Intents
     * created by our method `makeMessageIntentStack` and the flag FLAG_CANCEL_CURRENT (Flag
     * indicating that if the described PendingIntent already exists, the current one should be canceled
     * before generating a new one).
     *
     *
     * We initialize the String `String tickerText` by using our resource string
     * R.string.incoming_message_ticker_text as the format for our `message`. We create a
     * `Notification.Builder notifBuilder` for `NotificationChannel` PRIMARY_CHANNEL and
     ...
     *
     *
     * Finally we use `NotificationManager nm` to post the notification built from
     * `notifBuilder` in the status bar.
     */
     ...
     * initialize `NotificationManager nm`. We initialize `NotificationChannel chan1` with
     * a new instance whose id and user visible name are both PRIMARY_CHANNEL ("default"), and whose
     * importance is IMPORTANCE_DEFAULT (shows everywhere, makes noise, but does not visually intrude).
     * We set the notification light color of `chan1` to GREEN, and set its lock screen visibility
     * to VISIBILITY_PRIVATE (shows this notification on all lockscreens, but conceal sensitive or
     * private information on secure lockscreens). We then have `nm` create notification channel
     * `chan1`. We create a fake message to receive consisting of a `CharSequence from`
     * and a random `CharSequence message`. Next we create a `Intent intent` which will
     * launch the Activity `IncomingMessageInterstitial` when our notification is clicked. We
     * store our `from` as an extra in the Intent using the key KEY_FROM, and `message`
     * as an extra using the key KEY_MESSAGE. We set the flags of the Intent for FLAG_ACTIVITY_NEW_TASK
     * and FLAG_ACTIVITY_CLEAR_TASK. Next we create a new instance of `PendingIntent contentIntent`
     * which will launch the `intent` we just created and configured, we initialize the String
     * `String tickerText` by using our resource string R.string.incoming_message_ticker_text
     * as the format for our `message`. We create a `Notification.Builder notifBuilder`
     * for `NotificationChannel` PRIMARY_CHANNEL and chain together methods which:
     *
     *  *
     * Set the small icon to R.drawable.stat_sample
     *
    ...
    companion object {
         ...
         *
         *
         * First we create an array to hold 4 Intents `Intent[] intents`. The 0'th `Intent`
         ...
         * `Activity IncomingMessageView`, with our parameter `CharSequence from` stored as
         * an extra under the key KEY_FROM ("from"), and our parameter `CharSequence msg` stored
         * as an extra under the key KEY_MESSAGE ("message"). Finally we return `intents` to the
         * callers who use it as the argument to `startActivities(stack)`.
         *
         *
         * Note: `startActivities(stack)`, will Launch multiple new activities. This is generally
