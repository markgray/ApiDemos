...
class InstallApkSessionApi : AppCompatActivity() {
     ...
     * `Button button` by finding the view with id R.id.install and set its `OnClickListener`
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
         ...
         * variable `PackageInstaller.Session session` to null, then wrapped in a try block
         * intended to catch IOException in order to convert it to a RuntimeException, and catch
         * RuntimeException in order to abandon `session` if it is not null and rethrow the
         * RuntimeException we:
         *
         *  *
         * Initialize `PackageInstaller packageInstaller` with the interface that
         ...
         *
         *  *
         * Initialize `PackageInstaller.SessionParams params` with a new instance
         ...
         *
         *  *
         * Use `packageInstaller` to create a new package installer session saving
         * the id of the session in `int sessionId`.
         *
         *  *
         * Use `packageInstaller` to open session `sessionId` to actively
         * perform work, saving the `Session` instance in `session`.
         *
         *  *
         * Call our method `addApkToInstallSession` to read our sample APK assets
         * file "HelloActivity.apk" and write it into `session` under the name
         * "package".
         *
         *  *
         * Initialize `Context context` with this `InstallApkSessionApi` context.
         *
         *  *
         * Initialize `Intent intent` with a new instance intended for our activities
         * class `InstallApkSessionApi.class`, set its action to PACKAGE_INSTALLED_ACTION
         * and use it as the `Intent` that will be fired by `PendingIntent pendingIntent`
         * using a request code of 0 and no flags.
         *
         *  *
         * Initialize `IntentSender statusReceiver` with an `IntentSender`
         * object that wraps the existing sender of `PendingIntent pendingIntent`,
         * and use it as the status receiver callback when we commit `session`
    ...
    /**
     * Reads the file in our assets with the filename `String assetName` and writes it to the
     * `PackageInstaller.Session session`. Wrapped in a try block with the closeable asset
     * consisting of `OutputStream packageInSession` (a stream to write an APK file into the
     * PackageInstaller Session `session` using the name "package") and `InputStream is`
     * (an `InputStream` for the file with file name `String assetName`) we:
     *
     *  *
     ...
     *
     *  *
     * Loop while there is data to be read from `is` into `buffer` saving the number
     * of bytes read in `n` then writing `n` bytes from `buffer` into
     * `packageInSession`.
     *
     *
     *
    ...
    private fun addApkToInstallSession(assetName: String, session: PackageInstaller.Session) { // It's recommended to pass the file size to openWrite(). Otherwise installation may fail
...
// in onNewIntent().
     ...
     *
     *
     * First we initialize `Bundle extras` by fetching the extras stored in our parameter
     * `Intent intent`. If the action of `intent` is PACKAGE_INSTALLED_ACTION then we
     * initialize `int status` with the int stored in `extras` under the key EXTRA_STATUS
     * (status of the operation), and `String message` with the string stored in `extras`
     * under the key EXTRA_STATUS_MESSAGE (detailed string representation of the status, including
     * raw details that are useful for debugging). Then we switch on the value of `status`:
     *
     *  *
     * STATUS_PENDING_USER_ACTION: User action is currently required to proceed. We initialize
     * `Intent confirmIntent` by fetching the intent stored in `extras` under the
     ...
     * of `status` concatenated to `message` then break.
     *
     *  *
     * default: We toast a message consisting of the string "Unrecognized status received
     * from installer: " concatenated to `status`.
     *
     *
     *
