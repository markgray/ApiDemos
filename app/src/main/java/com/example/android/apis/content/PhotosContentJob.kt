...
class PhotosContentJob : JobService() {
    companion object {
         ...
         * We initialize `JobScheduler js` with a handle to the system level service which has the
         * class `JobScheduler.class`. We then use `js` to schedule the job described by the
         * prebuilt `JobInfo JOB_INFO` and log the fact that we have scheduled the `PhotosContentJob`.
         *
         ...
         * returning true if the job id of the i'th entry is `JobIds.PHOTOS_CONTENT_JOB`. If none of
         ...
         * `JobScheduler.class`, then use it to cancel the job with id `JobIds.PHOTOS_CONTENT_JOB`.
         *
         * @param context `Context` to use to get a handle to the `JobScheduler`.
         */
     ...
     * `PhotosContentJob` to run, then we call the `jobFinished` to inform the
     ...
     * the string "(No photos content)" to `sb`. If it is not null we initialize our flag
     * `boolean rescanNeeded` to false (this flag if true is used to flag the fact that the data
     * that we read from our parameter `JobParameters params` suggest that to properly list the
     * content changes will require a rescan of the photos, in which case we append the string "Photos
     * rescan needed!" to `sb`). If the list of URIs that have triggered the job returned by the
     * `getTriggeredContentUris` method of `params` is not null we want to iterate through
     * them and collect either the ids that were impacted or note that a generic change has happened,
     * so we initialize `ArrayList<String> ids` with a new instance then for each `Uri uri`
     * in the list of URIs that have triggered the job returned by the `getTriggeredContentUris`
     * method of `params` we initialize `List<String> path` with the decoded path segments
     * of `uri`, each without a leading or trailing '/'. If `path` is not null and its size
     * is one more than the size of EXTERNAL_PATH_SEGMENTS then the last entry in `path` is a
     * filename so we add that last entry to our list `ids`, otherwise there is some general
     * change in the photos so we set `rescanNeeded` to true.
     *
     *
     * When done processing the changed URIs in `params`, we check if the size of `ids`
     * is greater than 0 (we found some ids that changed), and if so we proceed to determine what they
     * are. To do this we initialize `StringBuilder selection` with a new instance then loop over
     * i for all the entries in `ids` first appending the string " OR " to `selection` following
     * every selection clause in `selection` (by skipping this if the size of `selection`
     * is 0). We then append the string "_id" followed by the string "='" followed by the i'th entry
     * in `ids` followed by the string "'" (forming a selection query like _id='88').
     *
     *
     * When done forming our selection string in `selection` we initialize `Cursor cursor`
     * to null and `boolean haveFiles` to false. Then wrapped in a try block intended to catch
     * SecurityException, and whose finally block closes `Cursor cursor` if it is not null we:
     *
     *  *
     ...
     * value of `selection` with null for the selection arguments and null for the
     * sort order saving the `Cursor` returned in `cursor`.
     *
     *  *
     * We now loop while the `moveToNext` method of `cursor` returns true when
     * moving to the next row (false is returned if it is past the last entry). We initialize
     * `String dir` by retrieving the string in column PROJECTION_DATA of `cursor`.
     * If `dir` starts with the string DCIM_DIR, we check if `haveFiles` is false
     * (first time through) and if so we set it to true and append the string "New photos:\n"
     * to `sb`. Then we append the string value of the int in column PROJECTION_ID of
     * `cursor` to `sb`, followed by the string ": ", followed by `dir`
     ...
     * `Runnable mWorker` to the queue of `Handler mHandler` we return true to the caller
