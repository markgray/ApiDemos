...
internal class KubeRenderer(
    ...
    /**
     * Called to draw the current frame. First we make sure our [AnimationCallback] field [mCallback]
     ...
     * depth buffer).
     *
     * Finally we call the `draw` method of [mWorld] to issue the final openGL commands to draw the
     * rubic cube.
     *
     ...
     * at (0,0), a width of [width] and a height of [height]. Next we calculate the [Float]
     * aspect ratio `val ratio` to be `width/height`, specify GL_PROJECTION to be the
