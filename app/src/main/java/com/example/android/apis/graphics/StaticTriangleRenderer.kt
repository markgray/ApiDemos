...
class StaticTriangleRenderer : GLSurfaceView.Renderer {
    ...
    /**
     * Set to a new instance of `Triangle` in our `init` method, whose `draw` method
    ...
    /**
     * `TextureLoader` whose `load` method is called to load the texture image we are to
     * use, it is set to one of the parameters to our method `init`. It is our own default
     * `RobotTextureLoader` as the `TextureLoader` when we are used by the activity
     * `TriangleActivity`, and depending on the compile time switch TEST_CREATE_TEXTURE either
     * `CompressedTextureLoader`, or `SyntheticCompressedTextureLoader` when we are used
     * by the activity `CompressedTextureActivity`
     */
    private var mTextureLoader: TextureLoader? = null

    /**
     * Our constructor needs to be provided with an implementation of this. We call its `load`
     * in our `onSurfaceCreated` callback to load the appropriate texture into our OpenGL
     * context.
     */
     ...
     * simply call our method `init` with our parameter `Context context` and a new
     * instance of `RobotTextureLoader` to use as its `CompressedTextureActivity`.
     *
    ...
    /**
     * Constructor that uses `TextureLoader loader` as its `TextureLoader`. It is used
     * in `CompressedTextureActivity`. We simply call our method `init` with our parameter
     * `Context context` to use for the context and `TextureLoader loader` to use as the
     * `TextureLoader`.
     *
     * @param context Context to use for resources, "this" `CompressedTextureActivity` when
     * called from the `onCreate` override of the `CompressedTextureActivity`
     * demo.
     ...
     *
     *
     * Now we create our texture. First we generate 1 texture name in our array `int[] textures`,
     * and we store the texture name created in our field `int mTextureID`, then we bind that
     ...
     * method of our field `TextureLoader mTextureLoader` to load the texture image.
     *
     ...
     *
     *
     * Now we calculate the `float angle` we want to use to rotate our triangle (using an
     * arbitrary function of the system uptime), and rotate our modelview matrix by that angle.
     * Finally we instruct our `Triangle mTriangle` to draw itself.
     *
     * @param gl the GL interface. Use `instanceof` to
     * test if the interface supports GL11 or higher interfaces.
     */
     ...
     *
     *
     * First we calculate the ratio of `w/h` which we will use to specify the coordinates for
     ...
     * we call the method `glFrustumf` to multiply the current matrix by a perspective matrix
     * built using `-ratio` as the left vertical clipping plane, `ratio` as the right
     * vertical clipping plane, `-1` for the bottom clipping plane, `1` for the top
     * clipping plane, `3` for the near clipping plane, and `7` for the far clipping
    ...
    private inner class RobotTextureLoader : TextureLoader {
         ...
         * current context into GL_TEXTURE_2D. First we open a data stream `InputStream is`
         * for reading the raw resource png image R.raw.robot. We declare `Bitmap bitmap` and
         * read and decode `is` into it. We use the utility method `GLUtils.texImage2D`
    ...
    internal class Triangle {
        ...
        /**
         * `ShortBuffer` loaded with the three indices 0, 1, and 2. It is used in our
         * `draw` method in the call to `glDrawElements` to specify the indices
         * which it uses to construct a sequence of geometric primitives that are drawn using
         * the vertices loaded from `mFVertexBuffer` and texture vertices loaded from
         * `mTexBuffer`
         */
         ...
         * `FloatBuffer mFVertexBuffer` as the pointer to the first coordinate of the first
         * vertex in the array.
         *
         *
         ...
         * `FloatBuffer mTexBuffer` as the pointer to the first coordinate of the first
         * vertex in the array.
         *
         *
         * Finally we call `glDrawElements` to render primitives from our array data, using
         * the primitive type GL_TRIANGLE_STRIP, VERTS (3) elements to be rendered, GL_UNSIGNED_SHORT
         * as the type of the values in our indices buffer, and `ShortBuffer mIndexBuffer` as
        ...
        fun draw(gl: GL10?) {
            ...
            GLES10.glDrawElements(GLES10.GL_TRIANGLE_STRIP, VERTS, GLES10.GL_UNSIGNED_SHORT, mIndexBuffer)
        }
         ...
         * we use to draw a triangle: `FloatBuffer mFVertexBuffer`, `FloatBuffer mTexBuffer`,
         * and `ShortBuffer mIndexBuffer`.
         *
         *
         * First: we allocate a direct byte buffer on the native heap for `ByteBuffer vbb`, and
         * set its byte order to native order. We use `vbb` to create a view of this byte
         * buffer as a float buffer which we store in the field `FloatBuffer mFVertexBuffer`.
         *
         *
         * Second: we allocate a direct byte buffer on the native heap for `ByteBuffer tbb`, and
         * set its byte order to native order. We use `tbb` to create a view of this byte
         * buffer as a float buffer which we store in the field `FloatBuffer mTexBuffer`.
         *
         *
         * Third: we allocate a direct byte buffer on the native heap for `ByteBuffer ibb`, and
         * set its byte order to native order. We use `ibb` to create a view of this byte
         * buffer as a short buffer which we store in the field `ShortBuffer mIndexBuffer`.
         *
         *
         * In `float[] coords` we declare the coordinates of a unit-sided equilateral triangle
         * centered on the origin.
         *
         *
         * We load each of the three, three dimensional points in `float[] coords` into
         * `FloatBuffer mFVertexBuffer` (each coordinate scaled by 2.0 for some reason).
         * We load the x and y coordinates of each of the three points in `float[] coords` into
         * `FloatBuffer mTexBuffer` (each coordinate scaled by 2.0 and offset by 0.5 for some
         * reason). And we load the three indices 0, 1, and 2 into `ShortBuffer mIndexBuffer`.
         *
         *
         * Finally we set each of the buffers `mFVertexBuffer`, `mTexBuffer` and
         * `mIndexBuffer` position to 0, so that the `gl*Pointer()` functions can read
