...
class LoaderThrottle : AppCompatActivity() {
     ...
     * `onCreate`. We fetch a handle to the FragmentManager for interacting with fragments
     * associated with this activity to `FragmentManager fm`. Then we use `fm` to search
     ...
     * of `ThrottledLoaderListFragment list` and use `fm` to start a `FragmentTransaction`
     * which we use to add `list` to the activity state using the ID android.R.id.content and
     ...
     *
     * @param context Context the `ContentProvider` `SimpleProvider` is running in
     */
    (context: Context)// calls the super constructor, requesting the default cursor factory.
         ...
         * NotePad class. We simply use our parameter `SQLiteDatabase db` to execute the
         ...
         * columns given by:
         *
         *  * MainTable._ID ("_id") an INTEGER PRIMARY KEY column
     ...
     * content provider, a subclass of `ContentProvider`. This should be a fully qualified
    ...
    class SimpleProvider : ContentProvider() {
        ...
        /**
         * Global provider initialization. We initialize our field `UriMatcher mUriMatcher` with
         ...
         * to `mUriMatcher` to match the authority AUTHORITY ("com.example.android.apis.app.LoaderThrottle"),
         ...
         * when a URI matches it. We initialize our field `HashMap<String, String> mNotesProjectionMap`
         * with an empty `HashMap<>`, then put the String MainTable._ID to map to itself, and
         ...
         *
         *
         * We simply initialize our field `DatabaseHelper mOpenHelper` with a new instance of
         ...
         * the `CursorLoader` to call this method when requesting more data.
         ...
         *
         *
         * First we create a new `SQLiteQueryBuilder qb` and set the list of tables to query to
         * `MainTable.TABLE_NAME` ("main" the only table in our pretend database). Then we switch
         * based on the return value of our `UriMatcher mUriMatcher` when matching the parameter
         * `Uri uri` (our app only uses the MAIN Uri, but the MAIN_ID option is included for
         * completeness (probably because all this code was pasted from another app)). In the "MAIN"
         * case we call `setProjectionMap` on our `SQLiteQueryBuilder qb` to set the
         * projection map for the query to `HashMap<String, String> mNotesProjectionMap`. The
         ...
         * the projection map to `mNotesProjectionMap`, then we append the chunk:
         * MainTable._ID + "=?"  to the WHERE clause of the query, and append the last path segment
         * of the `Uri uri` to the parameter `String[] selectionArgs`. (But since the
         * MAIN_ID case never occurs in our app, this is all just academic IMO). In either case we
         * continue by setting the `sortOrder` to MainTable.DEFAULT_SORT_ORDER if it was empty,
         * we use our field `DatabaseHelper mOpenHelper` to open the readable database
         * `SQLiteDatabase db`, create a `Cursor c` which performs a query of our
         * `SQLiteDatabase db` and Register to watch the content `URI uri` for changes.
         * (This can be the URI of a specific data row (for example, "content://my_provider_type/23"),
         * or a a generic URI for a content type.) Finally we return `Cursor c` to the caller.
         *
         ...
         *
         *
         * First we check to make sure that our parameter `Uri uri` is of the correct type
         ...
         *
         *
         * Next we initialize the variable `ContentValues values`, either with the value of our
         * parameter `ContentValues initialValues`, or an ,new empty set of values if
         ...
         *
         *
         * We then open (or create) `SQLiteDatabase db`, and try to insert the row that `values`
         * contains into `db` saving the row ID of the newly inserted row (or -1 if an error occurred)
         * in our variable `long rowId`. If no error occurred (`rowId > 0`) we create
         * `Uri noteUri` by appending `rowId` to our content URI base for a single row of data
         * (MainTable.CONTENT_ID_URI_BASE), notify registered observers that a row was updated and attempt
         * to sync changes to the network, and finally return `noteUri` to the caller. If an error
         * had occurred (`rowId == -1`) we throw an `SQLException` with an appropriate message.
         *
         *
         * See:
         *
         ...
         *
         *
         * First we open `SQLiteDatabase db`, then we allocate local variables `String finalWhere`
         * (used for Uri's containing a row ID (case MAIN_ID:), it is a SQL where clause we build
         * which includes the ID in with the parameter `String where` passed us), and `int count`
         * (used to save the return value of `SQLiteDatabase.delete` so we can return it to the
         * caller).
         *
         *
         * Next we branch based on the matching of `Uri uri` using our field `UriMatcher mUriMatcher`:
         *
         *  *
         * MAIN - we call the `delete` method of our `SQLiteDatabase db` for the
         * table TABLE_NAME ("main") and passing in the parameters `where` and `whereArgs`
         * unchanged. We save the return value from `delete` (the number of rows affected)
         * in `count` to later return to the caller. Note: in our app both `where`
         * and `whereArgs` are always null, so all rows in our database are deleted.
         *
         *  *
         * MAIN_ID - never used in our app, but included due to code pasting I guess. We create
         * the `String finalWhere` SQL command using the row ID parsed from our `Uri uri`
         * and call the `delete` method of our `SQLiteDatabase db` for the table
         * TABLE_NAME ("main"), using `finalWhere` as the WHERE parameter, and the
         * unmodified `whereArgs`. We save the return value from `delete` (the
         * number of rows affected) in `count` to later return to the caller.
         *
         *  *
         ...
         *
         *
         * First we allocate space for our variables `int count` (used to return the number of
         * rows affected by the call), and `String finalWhere` (used to construct a WHERE SQL
         * statement when a single row is specified in the parameter `Uri uri`).
         * Next we branch based on the matching of `Uri uri` using our field `UriMatcher mUriMatcher`:
         *
         *  *
         * MAIN - we call the `update` method of our `SQLiteDatabase db` for the
         * table TABLE_NAME ("main") and passing in the parameters `where` and `whereArgs`
         * unchanged. We save the return value from `update` (the number of rows affected) in
         * `count`.
         *
         *  *
         * MAIN_ID - We create the `String finalWhere` SQL command using the row ID parsed from
         * our `Uri uri` and call the `update` method of our `SQLiteDatabase db`
         * for the table TABLE_NAME ("main"), using `finalWhere` as the WHERE parameter, and the
         * unmodified `whereArgs`. We save the return value from `update` (the
         * number of rows affected) in `count` to later return to the caller.
         *
         *  *
         * default - We throw an `IllegalArgumentException`.
    ...
    class ThrottledLoaderListFragment : ListFragment(), LoaderManager.LoaderCallbacks<Cursor> {
         ...
         *
         *
         * Next we initialize our field `SimpleCursorAdapter mAdapter` with a new instance of
         ...
         * the `Cursor` (since we will create an assign the cursor later), the single column
         ...
         *
         *
         * We call `setListShown(false)` in order to start out with a progress indicator.
         *
         *
         ...
         * this for its `LoaderCallbacks` callbacks.
         *
         * @param savedInstanceState we do not override `onSaveInstanceState` so do not use this
         */
         ...
         * order, and "Clear" for its title. After adding each `MenuItem` set set their
         ...
         * ContentResolver instance for application's package: `ContentResolver cr`. Then we
         * switch based on the item ID of the `MenuItem item` selected:
         *
         *  *
         * POPULATE_ID - ("POPULATE" `MenuItem`) If there is already an existing
         * `AsyncTask<Void, Void, Void> mPopulatingTask` running, we cancel that task
         * specifying that it not be interrupted. Then we create a new `mPopulatingTask`
         * with the `doInBackground` override using `ContentResolver cr` to insert
         ...
         * we return true to the caller to indicate that we consumed the `MenuItem`
         * click here.
         *
         *  *
         * CLEAR_ID ("CLEAR" `MenuItem`) If there is already an existing
         * `AsyncTask<Void, Void, Void> mPopulatingTask` running, we cancel that task
         * specifying that it not be interrupted, then set `mPopulatingTask` to null.
         * Then we create `AsyncTask<Void, Void, Void> task` with the `doInBackground`
         * override using `ContentResolver cr` to delete the entire contents of the
         * MainTable.CONTENT_URI database. We start `task` executing in the background
         * and return true to the caller to indicate that we consumed the `MenuItem`
         * click here.
         *
         *  *
