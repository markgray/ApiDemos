...
class TextToSpeechActivity : AppCompatActivity(), OnInitListener {
     ...
     * We create an instance of `TextToSpeech` for `TextToSpeech mTts` using "this" as
     ...
     * our field `Button mAgainButton`, and we set its `OnClickListener` to an anonymous
     * class which calls our method `sayHello` when the Button is clicked.
     *
...
// It will be enabled upon initialization of the TTS engine.
     ...
     * for `TextToSpeech mTts`, we interrupt the current utterance and discard other utterances
     ...
     */
    public override fun onDestroy() { // Don't forget to shutdown!
        if (mTts != null) {
            mTts!!.stop()
            mTts!!.shutdown()
        }
     ...
     * method in the interface `TextToSpeech.OnInitListener`. If the `status` is not
     ...
     * `Button mAgainButton`, and call our method `sayHello()` to utter the first
     * random utterance.
     *
...
// int result mTts.setLanguage(Locale.FRANCE);
            ...
            } else { // Check the documentation for other possible result codes.
...
// Allow the user to press the button for the app to speak again.
    ...
    /**
     * Use the TextToSpeech engine to speak a random sentence. First we initialize `helloLength`
     * to the number of Strings in our `String[] HELLOS`, and then use it along with our random
     * number generator `Random RANDOM` to pick a String from `HELLOS[]` for
     * `String hello`, which we then pass to `speak` method of `TextToSpeech mTts`,
