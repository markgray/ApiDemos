...
class LocalServiceActivities {
    ...
    class Controller : AppCompatActivity() {
         ...
         * `Button button` by finding the view with id R.id.start ("Start Service") and set its
         * `OnClickListener` to our field `OnClickListener mStartListener`, then find for
         * `button` the view with id R.id.stop ("Stop Service") and set its `OnClickListener`
         * to our field `OnClickListener mStopListener`.
         *
        ...
        /**
         * `OnClickListener` for the `Button` with id id R.id.start ("Start Service"),
         * the `onClick` override starts the `LocalService` `Service`.
         */
        /**
        ...
        /**
         * `OnClickListener` for the `Button` with id id R.id.stop ("Stop Service"),
         * the `onClick` override stops the `LocalService` `Service`.
         */
        /**
    ...
    class Binding : AppCompatActivity() {
        ...
        private val mConnection: ServiceConnection = object : ServiceConnection {
             ...
             *
             *
             * We cast our parameter `IBinder service` to `LocalService.LocalBinder` and
             * call its `getService` method which returns a pointer to its 'this' which we save
             * in our field `LocalService mBoundService`. We then toast the string with resource
...
// cast its IBinder to a concrete class and directly access it.
             ...
             *
             *
             * We set our field `LocalService mBoundService` to null, and toast the string with
...
// see this happen.
        ...
        /**
         * Called from `OnClickListener mBindListener` which is the `OnClickListener` for
         * the button with id R.id.bind ("Bind Service"), this method attempts to establish a connection
         * with the service `LocalService.class`. We call the `bindService` method to bind
         * to `LocalService` giving `ServiceConnection mConnection` as the callback object
         ...
         * our flag `mShouldUnbind` to true, if it returns false we log an error message.
         */
        fun doBindService() { // Attempts to establish a connection with the service.  We use an
...
// applications).
         ...
         * with `ServiceConnection mConnection`, which is the same connection interface previously
         * supplied to `bindService`.
         */
        fun doUnbindService() {
            if (mShouldUnbind) { // Release information about the service's state.
        ...
        /**
         * `OnClickListener` for the button with id R.id.bind ("Bind Service"), the
         * `onClick` override we just calls our `doBindService` method to attempt to
         * establish a connection to the service `LocalService.class`.
         */
        /**
        ...
        /**
         * `OnClickListener` for the button with id R.id.unbind ("Unbind Service"), the
         * `onClick` override we just calls our `doUnbindService` method to disconnect
         * from the service `LocalService.class` if need be.
         */
        /**
         ...
         * `Button button` by finding the view with id R.id.bind ("Bind Service") and set its
         * `OnClickListener` to `mBindListener`, set `button` again by finding the
         * view with id R.id.unbind ("Unbind Service") and set its `OnClickListener` to
         * `mUnbindListener`. Finally we set `button` by finding the view with id
         * R.id.do_something ("Do something") and set its `OnClickListener` to an anonymous
         * class whose `onClick` override calls the `doSomeThing` method of our binding
         * to the service in `LocalService mBoundService` if it is not null.
         *
         * @param savedInstanceState we do not override `onSaveInstanceState` so do not use.
         */
        override fun onCreate(savedInstanceState: Bundle?) {
             ...
             * we use `mBoundService` to execute the method `LocalService.doSomeThing`.
             *
