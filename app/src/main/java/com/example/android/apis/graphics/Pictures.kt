...
class Pictures : GraphicsActivity() {
    ...
    private class SampleView(context: Context?) : View(context) {
        ...
        /**
         * `PictureDrawable` created out of `Picture mPicture`.
         */
        private val mDrawable: Drawable

        /**
         * We implement this to do our drawing. First we set the entire `Canvas canvas` to the
         * color WHITE. Then we draw our `Picture mPicture` 3 different ways:
         *
         *  *
         * Using the `canvas` method `drawPicture`
         *
         *  *
         * Using the `canvas` method `drawPicture` and specifying a `RectF`
         ...
         *
         *  *
         * Setting the bounds of our `Drawable mDrawable` version of `mPicture` to
         * left of 0, top of 200, right of `getWidth` and bottom of 300, then using the
         * `draw` method of `mDrawable` to draw to the `Canvas canvas`
         *
         *
         * Notice the three different ways the `Picture` is located on the canvas, using
         * `RectF` to move it down by 100 pixels, using `setBounds` to move it down by
         * 200 pixels, and using `translate` to move it down by 300 pixels.
         *
...
*/
        ...
        companion object {
             ...
             * `Paint` for `Paint p`, with the ANTI_ALIAS_FLAG set. We set the color of `p`
             * to red with an alpha of 0x88 and use it to draw a 40 pixel radius circle on `Canvas canvas`
             * with its center at (50,50). We set the color of `p` to GREEN, set its text size to 30,
             * and use it to draw the text "Pictures" at location (60,60) on `canvas`.
             *
             * @param canvas `Canvas` to draw to, it is created by calling the `beginRecording`
             * method of our field `Picture mPicture`.
             */
         ...
         * focus, and to receive focus in touch mode. We create a new instance of `Picture` for
         * our field `Picture mPicture`, call its `beginRecording` method specifying a
         * width of 200, and a height of 100, and pass the `Canvas` returned to our method
         * `drawSomething`. When `drawSomething` is done drawing to the `mPicture`
         * canvas we call the `endRecording` method of `mPicture`. Finally we create a
         * `PictureDrawable` from `mPicture` for our field `Drawable mDrawable`.
         *
         * Parameter: context `Context` to use to access resources, "this" when called from the
         * `onCreate` method of the `Pictures` activity.
         */
        init {
            isFocusable = true
