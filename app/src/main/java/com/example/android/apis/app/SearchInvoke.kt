...
class SearchInvoke : AppCompatActivity() {
    // UI elements
    var mStartSearch // Button used to start search - Without a keyboard, you need to press this to get a soft keyboard to use.
            : Button? = null
    var mMenuMode // Spinner used to select between "Search Key", "Menu Item", "Type-To-Search" or "Disabled"
            : Spinner? = null
    var mQueryPrefill // Used to enter text to prefill the search
            : EditText? = null
    var mQueryAppData // context specific data to include in a Bundle under the key "demo_key" it will be returned in search Intents
            : EditText? = null
     ...
     * We locate the `Button mStartSearch` R.id.btn_start_search, the `Spinner mMenuMode`
     * R.id.spinner_menu_mode, the `EditText mQueryPrefill` R.id.txt_query_prefill, and the
     * `EditText mQueryAppData` R.id.txt_query_appdata. Then we create from R.array.search_menuModes
     ...
     * and set the `SpinnerAdapter` of `Spinner mMenuMode` to `adapter`. We set the
     * `OnItemSelectedListener` of `mMenuMode` to an anonymous class which selects the
     ...
     * to tell the difference). Finally we set the `OnClickListener` of `Button mStartSearch`
     * to an anonymous class which calls our override of the method `onSearchRequested` when the
     * Button is clicked.
     *
...
// the default key mode in your activity's onCreate() handler.
     ...
     * super's implementation of `onPrepareOptionsMenu`. Then we declare a `MenuItem item`.
     ...
     *  * MENUMODE_MENU_ITEM - we add the MenuItem "Search" as item ID 0, and set its alphabetic shortcut to MENU_KEY
     ...
     * the selection of `Spinner mMenuMode`:
     *
     *  * MENUMODE_SEARCH_KEY - we pop up a dialog instructing the user to dismiss the dialog and use the search key
     *  * MENUMODE_MENU_ITEM - immediately calls our override of `onSearchRequested`
     *  * MENUMODE_TYPE_TO_SEARCH - we pop up a dialog instructing the user to dismiss the dialog and start typing
     *  * MENUMODE_DISABLED - we pop up a dialog instructing the user that they have disabled search
     *
     * After either of these menu items we return the super's implementation of `onOptionsItemSelected` to the caller.
     *
     ...
     * contents of `EditText mQueryPrefill` to `String queryPrefill` (the String the
     * user has typed in to prefill the search box). Next we declare `Bundle appDataBundle`,
     * fetch the contents of `EditText mQueryAppData` to `String queryAppDataString` (the
     * String the user has entered to be used for context-specific search data), and if it is not
     * null we allocate a new `Bundle` for `appDataBundle` and store `queryAppDataString`
     * in it using the key "demo_key". Next we call the `Activity` member function `startSearch`
     * using `queryPrefill` as the initial query, the flag false so that the query is not preselected,
     * `appDataBundle` as the application-specific context, and false to flag that it should
     ...
     */
    override fun onSearchRequested(): Boolean { // If your application absolutely must disable search, do it here.
...
// For most applications, you can simply pass null to startSearch().
     ...
     *
     * First we create an instance of `SearchRecentSuggestions suggestions` configured to
     * use `SearchSuggestionSampleProvider.AUTHORITY` ("com.example.android.apis.SuggestionProvider"),
     * and SearchSuggestionSampleProvider.MODE (DATABASE_MODE_QUERIES - configures the database to record
     * recent queries - required). Then we instruct `suggestions` to perform a clear history
     * operation.
     */
...
// This list must match the list found in samples/ApiDemos/res/values/arrays.xml
        const val MENUMODE_SEARCH_KEY = 0 // "Search Key" Uses the search key to launch searches (Needs keyboard)
        const val MENUMODE_MENU_ITEM = 1 // "Menu Item" Uses the menu item itself to launch searches
        const val MENUMODE_TYPE_TO_SEARCH = 2 // "Type-To-Search" unhandled keystrokes will start an application-defined search (needs keyboard)
        const val MENUMODE_DISABLED = 3 // "Disabled" Search is disabled
    }
}