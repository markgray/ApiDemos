...
class ShadowCardDrag : AppCompatActivity() {
     ...
     * `TriangleShape` which are contained in `ArrayList<Shape> mShapes`.
     */
     ...
     * is filled in our `onCreate` method with `RectShape`, `OvalShape`,
     * `RoundRectShape` and `TriangleShape`, and cycled through when the R.id.shape_select
     * "Select Shape" `Button` is pressed.
     */
     ...
     * method `initShapes` to fill our field `ArrayList<Shape> mShapes` with four
     * different `ShapeDrawable` objects: a `RectShape`, an `OvalShape`, a
     * `RoundRectShape`, and a `TriangleShape`. We fetch the `Paint` used to draw
     * our field `ShapeDrawable mCardBackground` and set its color to WHITE, then set its
     * background to the first `Shape` in `mShapes` (a `RectShape`). We locate the
     * `TextView` in our layout with id R.id.card and set its background to `mCardBackground`.
     * We next initialize our "Enable Tilt" checkbox, our "Enable Shading" checkbox, and our "Select
     * Shape" button. Finally we initialize an `OnTouchListener` for our entire `FrameLayout`
    ...
    /**
     * Initializes the `OnTouchListener` of the `FrameLayout` to allow the user to move
     * the card with his finger. First we set `View cardParent` to the view in our layout with
     * id `R.id.card_parent` (the `FrameLayout` holding our entire UI). Then we set its
     * `OnTouchListener` to an anonymous class which will translate the `MotionEvent`
     ...
     */
        ...
        cardParent.setOnTouchListener(object : OnTouchListener {
             ...
             * action of the `MotionEvent event`:
             *
             *  *
             * ACTION_DOWN - First we calculate how far the event coordinates are from the
             * current location of the card, setting `downX` and `downY` so that
             * they may be used to calculate where to move the card when future ACTION_MOVE
             * events are received. Then we create an `ObjectAnimator upAnim` to animate
             * the property name "translationZ" of `View mCard`, set its duration to
             * 100 milliseconds, its interpolator to a new instance of `DecelerateInterpolator`,
             * and then start it running. If the flag `mTiltEnabled` is true, we call the
             * `onDown` method of `CardDragState mDragState` to initialize it for
             ...
             * true we call the `onMove` method of `CardDragState mDragState` with
             ...
             *
             *  *
             * ACTION_UP - We create `ObjectAnimator downAnim` to animate the "translationZ"
             * attribute of `View mCard` to 0, set its duration to 100 milliseconds, set
             * its interpolator to a new instance of `AccelerateInterpolator`, and start
             * the animation running. Then is our flag `mTiltEnabled` is true we call the
             * `onUp` method of `CardDragState mDragState` to animate the flattening
    ...
    /**
     * Sets the `OnClickListener` of the "Select Shape" button (id R.id.shape_select) to an
     * anonymous class which steps through the list of `Shape` objects in `ArrayList<Shape> mShapes`
     * setting the `ShapeDrawable mCardBackground` to a different one every time it is clicked.
     */
    private fun initShapeButton() {
        ...
        shapeButton.setOnClickListener(object : View.OnClickListener {
             ...
             * of `Shape` objects in the list `ArrayList<Shape> mShapes`, then we fetch
             * the `Shape` at index `index` and use it to set the `ShapeDrawable`
             * of `ShapeDrawable mCardBackground`.
             *
    ...
    /**
     * Sets the `OnClickListener` of the "Enable Shading" checkbox (id R.id.shading_check) to an
     * anonymous class which saves the value of `boolean isChecked` in `mShadingEnabled`
     * and if it is not enabled now, sets the color filter of `ShapeDrawable mCardBackground`
     * to null.
     */
    ...
    private fun initShadingEnable() {
        ...
        shadingCheck.setOnCheckedChangeListener { buttonView, isChecked ->
             ...
             * of `boolean isChecked` in `mShadingEnabled` and if it is not enabled now,
             * we set the color filter of `ShapeDrawable mCardBackground` to null.
             *
    ...
    /**
     * Sets the `OnClickListener` of the "Enable Tilt" checkbox (id R.id.tilt_check) to an
     * anonymous class which saves the value of `boolean isChecked` in `mTiltEnabled`
     * and if it is not enabled now, calls the `onUp` method of `CardDragState mDragState`
    ...
    private fun initTiltEnable() {
        ...
        tiltCheck.setOnCheckedChangeListener { buttonView, isChecked ->
            /**
             * Called when the checked state of a compound button has changed. First we save the value
             * of `boolean isChecked` in `mTiltEnabled` and if it is not enabled now,
             * we call the `onUp` method of `CardDragState mDragState` to animate the
    ...
    /**
     * Fills the list of `Shape` objects in `ArrayList<Shape> mShapes` with the following
     * new instances: a `RectShape`, an `OvalShape`, a `RoundRectShape` and a
     * `TriangleShape`.
     */
    ...
    private inner class CardDragState {
        /**
         * Time of the last event we were informed of when our `onDown` or `onMove` methods
        ...
        /**
         * X location of the last event we were informed of when our `onDown` or `onMove`
        ...
        var lastX = 0f
        /**
         * Y location of the last event we were informed of when our `onDown` or `onMove`
         * methods were called. Used to calculate the speed that the card is being moved at in order
         * to scale the "tilt" and/or "shading" of the card proportionately.
         */
        var lastY = 0f
        ...
        /**
         * Called when the `OnTouchListener` of our `FrameLayout` (id R.id.card_parent)
         * receives an ACTION_DOWN event and only if tilt is enabled. We save the event time
         * `eventTime` in our field `lastEventTime`, and the `x` and `y`
         * coordinates of the event in our fields `lastX` and `lastY` respectively. We
         * then set both `momentumX` and `momentumY` to 0.
         *
         ...
         * in time `deltaT` of the event time since `lastEventTime` was set. If this is
         * not 0, we calculate and scale values for `momentumX` and `momentumY` based on
         * the movement since the last event, and rotate the card about the X axis proportionately
         * to the value of `-momentumY` and rotate the card about the Y axis proportionately
         * to the value of `-momentumX`. Then if shading is enabled, we calculate a value of
         * `float alphaDarkening` proportional to the momentum, scale it into a byte, and use
         * that byte to create an rgb color which we set as the color filter using PorterDuff MULTIPLY
         * mode for `ShapeDrawable mCardBackground`. Finally we save the event time in our field
         * `lastEventTime`, and the `x` and `y` coordinates of the event in our
         * fields `lastX` and `lastY` respectively.
         *
         ...
         * `OnCheckedChangeListener` of the "Enable Tilt" checkbox when tilt is disabled. First
         * we create `ObjectAnimator flattenX` to animate the "rotationX" attribute of our
         * `View mCard` to 0 rotation, set its duration to 100  milliseconds, its interpolator
         * to an `AccelerateInterpolator`, then start it running. Next we create
         * `ObjectAnimator flattenY` to animate the "rotationY" attribute of our
         * `View mCard` to 0 rotation, set its duration to 100  milliseconds, its interpolator
         * to an `AccelerateInterpolator`, then start it running. Finally we set the color
         * filter of `ShapeDrawable mCardBackground` to null.
         */
        fun onUp() {
    ...
    private class TriangleShape : Shape() {
        ...
        /**
         * Callback method called when [.resize] is executed. First we clear any
         * lines and curves from the `Path mPath`, making it empty. We move the path to (0,0),
         * draw a line to (`width`,0), draw a line to `(width/2,height)`, then draw a
         ...
         * `drawPath` method of `Canvas canvas` to draw `Path mPath` using
         * `Paint paint`.
         *
         ...
         * call the `setConvexPath` method of `Outline outline` to construct an outline
         * of our `Path mPath`.
         *
