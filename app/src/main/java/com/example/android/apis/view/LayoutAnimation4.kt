...
/**
 * Its layout file R.layout.layout_animation_4 uses a android:layoutAnimation attribute to animate
 ...
 */
class LayoutAnimation4 : AppCompatActivity() {
    /**
     * Called when the activity is starting. First we call through to our super's implementation of
     * `onCreate`, then we set our content view to our layout file R.layout.layout_animation_4.
     ...
     * action MAIN, and category LAUNCHER). We initialize our [GridView] variable `grid` by finding
     * the view with the ID R.id.grid, and set its adapter to a new instance of [AppsAdapter].
     *
    ...
    inner class AppsAdapter : BaseAdapter() {
         ...
         * create a new instance for our [ImageView] variable `val i`. We initialize our [ResolveInfo]
         * variable `val info` with the data in [mApps] at position [position] modulo the size of
         * [mApps], and set the content of `i` to a drawable of the icon associated with `info` that
         * we retrieve by using that [ResolveInfo] to call back a `PackageManager` instance to load
         * the icon from the application. We set the scale type of `i` to FIT_CENTER. We set our
         * variable `val w` to 36 times the logical display density plus 0.5, and use it to set the
         * layout parameters of `i` to `w` by `w`. Finally we return `i` to the caller.
         *
        ...
        override fun getView(position: Int, convertView: View?, parent: ViewGroup): View {
            val i = ImageView(this@LayoutAnimation4)
         ...
         * the size of the [mApps] field and 32.
         *
         ...
         * data at position [position] modulo the size of the [mApps] field.
         *
