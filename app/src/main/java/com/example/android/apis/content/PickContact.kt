...
class PickContact : AppCompatActivity() {
    ...
    /**
     * `ResultDisplayer` instance that launched contact data base query, it is set to "this"
     * in the `onClick` override of `ResultDisplayer` when the Button using "this"
     * `ResultDisplayer` as its `OnClickListener` is clicked, and used in our
     * `onActivityResult` when the `Intent` launched returns a result in order to access
     * the field `ResultDisplayer.mMsg` to use in the toast.
     */
    var mPendingResult: ResultDisplayer? = null

    /**
     * This is a custom `OnClickListener` which is used for each of the `Button`'s in our
     * UI. It uses its constructor to specify a string to be used as part of the toast which will be
     * shown by `onActivityResult`, as well as the MIME data type used in the `Intent`
     * which launches the contact data base query.
     */
    inner class ResultDisplayer
    /**
     * Constructor used for each Button in our UI, we save the parameter `String msg` in our
     * field `String mMsg` and `String mimeType` in `String mMimeType`
     *
     * Parameter: msg      String to use as part of the toast displayed when the result of the contact
     * database query is returned to `onActivityResult`.
     * Parameter: mimeType MIME data type to use for the `Intent` launching the contact
     * database query.
     */(
            /**
             * String to use as part of the toast displayed when the result of the contact database query
             * is returned to `onActivityResult`.
             */
            var mMsg: String,
            /**
             * MIME data type to use for the `Intent` launching the contact database query.
             */
            var mMimeType: String) : View.OnClickListener {

        /**
         * Called when the Button is clicked that uses this instance of `ResultDisplayer` as
         * its `OnClickListener`. First we create `Intent intent` with the action
         ...
         * `String mMimeType` (which is set for this instance in our constructor), set our
         * Activity's field `ResultDisplayer mPendingResult` to this so that we can be found
         * by `onActivityResult` when a result for a query is returned, and finally we use
         * `intent` to launch an activity for which we would like a result when it finishes.
         * (Said result will be returned to `onActivityResult` in the sweet by-and-by).
         *
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        findViewById<View>(R.id.pick_contact).setOnClickListener(ResultDisplayer("Selected contact", CONTACT))
        findViewById<View>(R.id.pick_person).setOnClickListener(ResultDisplayer("Selected person", PERSON))
        findViewById<View>(R.id.pick_phone).setOnClickListener(ResultDisplayer("Selected phone", PHONE))
        findViewById<View>(R.id.pick_address).setOnClickListener(ResultDisplayer("Selected address", POSTAL))
    }
     ...
     *
     *
     * If our parameter `Intent data` is non-null, we retrieve the `Uri uri` data this
     * intent is operating on. If `uri` is non-null we allocate `Cursor c`, setting it
     * to null, then wrapped in a try block we use a ContentResolver instance for our application's
     * package to query `uri` for the column containing the unique ID for a row, saving the
     * `Cursor` returned in `Cursor c`. If `c` is now non-null and we can
     * successfully move to the first row of `c` we retrieve the `int` value of column
     * 0 of that row to `int id`. If there has been a toast already toasted using our field
     * `Toast mToast` we cancel it.
     *
     *
     * We now create `String txt` using our field `ResultDisplayer mPendingResult` to
     * retrieve the value of the field `String mMsg` of the `ResultDisplayer` instance
     * whose result we are receiving after its Button was clicked, concatenated with the string value
     * of `uri` and the value of `id`. We set `Toast mToast` to a toast that uses
     * `txt` as its text and show it.
     *
     *
