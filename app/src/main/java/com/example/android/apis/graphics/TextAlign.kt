...
class TextAlign : GraphicsActivity() {
    ...
    private class SampleView(context: Context?) : View(context) {
         ...
         * the parameter `String text`. First we allocate an entry for every character in
         * `text` in `float[] widths`. Then we fill it with the widths of all of the
         * characters in `text`, saving the number of code units in the text in `int n`.
         * We allocate for `float[] pos` an array of `2*n` `float` objects.
         *
         *
         * Then initializing `accumulatedX` to 0, we loop through all `n` values in
         * `widths` setting two entries in `pos` for the coordinates of each character,
         * the first value being `accumulatedX`, and the second our parameter `y`. We
         * then add the entry in `widths` for this character to `accumulatedX` and loop
        ...
        /**
         * We implement this to do our drawing. First we draw the entire `Canvas canvas` WHITE.
         * We make a copy of the `Paint mPaint` pointer for `Paint p`, copy `mX` to
         * `float x`, set `y` to 0, and make a copy of the `float[] mPos` pointer in
         * `float[] pos`.
         *
         *
         ...
         * the color of `p` to BLACK, translate the `canvas` down `DY` pixels, set
         * the text alignment of `p` to LEFT, and use it to draw the text TEXT_L ("Left") at
         * location (x,y). We move the `canvas` down `DY` pixels, set the text alignment
         * of `p` to CENTER, and use it to draw the text TEXT_C ("Center") at location (x,y).
         * We move the `canvas` down `DY` pixels, set the text alignment of `p` to
         * RIGHT, and use it to draw the text TEXT_R ("Right") at location (x,y).
         *
         *
         * Now we move on to the positioned text, first moving the `Canvas canvas` to the
         * location (100,DY*2), and setting the color of `p` to green. We loop through the
         * coordinates in `pos` drawing a vertical line between one `DY` above to two
         * `DY` below the (x,y) coordinates contained in `pos`.
         *
         *
         * Then we set the color of `p` to BLACK, its text alignment to LEFT, and use the method
         * `canvas.drawPosText` to draw the text `POSTEXT` ("Positioned"), using `pos`
         * as the location of each character, and the `Paint p` as the `Paint`. We move the
         * `Canvas canvas` down by `DY`, set the text alignment of `p` to to CENTER,
         * and use the method `canvas.drawPosText` to draw the text `POSTEXT` ("Positioned"),
         * using `pos` as the location of each character, and the `Paint p` as the `Paint`.
         * Finally we move the `Canvas canvas` down by `DY`, set the text alignment of `p`
         * to to RIGHT, and use the method `canvas.drawPosText` to draw the text `POSTEXT`
         * ("Positioned"), using `pos` as the location of each character, and the `Paint p`
         * as the `Paint`.
         *
         *
         * We move the `Canvas canvas` to (-100,DY*2) (relative to its last position of course),
         * and draw our `Path mPath` using the `Paint mPathPaint`. We set the text alignment
         * of `p` to LEFT, and use the method `canvas.drawTextOnPath` to draw the text
         * TEXTONPATH ("Along a path") along the `Path mPath` using `Paint p` as the
         * `Paint`. We move the `Canvas canvas` down by `DY*1.5` pixels, and draw
         * our `Path mPath` using the `Paint mPathPaint`. We set the text alignment of
         * `p` to CENTER, and use the method `canvas.drawTextOnPath` to draw the text
         * TEXTONPATH ("Along a path") along the `Path mPath` using `Paint p` as the
         * `Paint`. And finally we move the `Canvas canvas` down by `DY*1.5` pixels,
         * and draw  our `Path mPath` using the `Paint mPathPaint`. We set the text alignment
         * of `p` to RIGHT, and use the method `canvas.drawTextOnPath` to draw the text
         * TEXTONPATH ("Along a path") along the `Path mPath` using `Paint p` as the
         * `Paint`.
         *
         ...
         * super's implementation of `onSizeChanged`, then we initialize our field `mX` to
         * half of the new width of the view `int w`.
         *
        ...
        companion object {
            ...
            /**
             * Fills its parameter `Path p` with a cubic bezier from (10,0) to (300,0) using the
         ...
         * focus. We initialize our field `Paint mPaint` with a new instance of `Paint`,
         * set its anti alias flag to true, its text size to 30, and its typeface to SERIF. We initialize
         * our field `float[] mPos` with the character positions that the characters in the
         * string POSTEXT ("Positioned") when it is drawn. We allocate a new instance of `Path`
         * for `Path mPath` and call our method `makePath` to fill `mPath` with a
         * cubic bezier from (10,0) to (300,0). Then we allocate a new instance of `Paint` for
         * `Paint mPathPaint`, set its anti alias flag to true, set its color to blue (with an
         * alpha of 0x80), and set its style to STROKE.
         *
         * context `Content` to access resources, "this" in the `onCreate` method
         * of `TextAlign`.
         */
        init {
            isFocusable = true
