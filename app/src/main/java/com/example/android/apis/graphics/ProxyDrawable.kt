...
/**
 * Saves the parameter `Drawable target` in our field `Drawable mProxy`, so it can
 * be retrieved using our method `getProxy` in the `draw` override of our subclass
 *
 * Parameter: target `Drawable` we are to proxy for, thereby allowing a subclass of us to override
...
(
     ...
     * `Drawable mProxy` we proxy for (it and "this" are now mutable)
     */
    private var mMutated = false

    /**
     * Draw in its bounds (set via setBounds) respecting optional effects such as alpha (set via
     * setAlpha) and color filter (set via setColorFilter). If our field `Drawable mProxy` is
     * not null, we pass the call to it. This is overridden by out subclass `AnimateDrawable`
     *
     ...
     * width, such as with a solid color. If our field `Drawable mProxy` is not null, we pass
     * the call through to it, otherwise we return -1.
     *
     * @return intrinsic width of our field `Drawable mProxy`
     */
     ...
     * height, such as with a solid color. If our field `Drawable mProxy` is not null, we pass
     * the call through to it, otherwise we return -1.
     *
     * @return intrinsic height of our field `Drawable mProxy`
     */
     ...
     *
     *
     * If our field `Drawable mProxy` is not null, we pass the call through to its
     ...
     *
     *
     * If our field `Drawable mProxy` is not null we pass the call through to it.
     *
     ...
     * filter disables [tint][android.graphics.drawable.Drawable.setTintList].
     *
     *
     * If our field `Drawable mProxy` is not null we pass the call through to it.
     *
     ...
     * opaque. If our field `Drawable mProxy` is not null we pass the call through to it.
     *
     ...
     *
     *
     * If our field `Drawable mProxy` is not null, and our field `boolean mMutated` is
     * false (meaning we have not been called already), we check to see if our super's implementation
     * of `mutate` returns a reference to "this" (which it should!) and if we pass these tests
     * we pass the call through to `mProxy` and set the `mMutated` flag to true.
     *
