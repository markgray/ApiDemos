...
class Animation2 : AppCompatActivity(), OnItemSelectedListener {
     ...
     * initialize our field `ViewFlipper mFlipper` by locating the `ViewFlipper` with ID
     * R.id.flipper in our layout file, and instruct it to "start flipping" (starts a timer for it to
     * cycle through its child views).
     *
     *
     * Next we set `Spinner s` by locating the view with ID R.id.spinner, then we create
     * `ArrayAdapter<String> adapter` from our field `String[] mStrings` using the
     ...
     *
     *  *
     * 0 - "Push up" we set the in animation of `ViewFlipper mFlipper` to the `Animation`
     * loaded from R.anim.push_left_in, and its out animation to one loaded from R.anim.push_left_out
     *
     *  *
     * 1 - "Push left" we set the in animation of `ViewFlipper mFlipper` to the `Animation`
     * loaded from R.anim.push_up_in, and its out animation to one loaded from R.anim.push_up_out
     *
     *  *
     * 2 - "Cross fade" we set the in animation of `ViewFlipper mFlipper` to the `Animation`
     * loaded from android.R.anim.fade_in, and its out animation to one loaded from android.R.anim.fade_out
     *
     *  *
     * default - "Hyperspace" we set the in animation of `ViewFlipper mFlipper` to the `Animation`
     ...
     * and a duration of 400 milliseconds, the `<rotate>` rotates from 0 degrees to -45 degrees,
