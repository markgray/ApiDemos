...
class IsolatedServiceController : AppCompatActivity() {
    ...
    class ServiceInfo(
        ...
        /**
         * Called from the `onDestroy` callback of the [IsolatedServiceController] activity. If we
        ...
        /**
         * `OnClickListener` for the Button with resource ID `start`, we create an [Intent] to start
         * the service with the [Class] in our field [mClz] and start it when the Button is clicked.
         */
         ...
         * already bound (our [mServiceBound] field is true), and if not we bind to the service
         * with the [Class] in our field [mClz] using our [ServiceConnection] field [mConnection]
         * as the [ServiceConnection] object, set our flag [mServiceBound] to *true*, and set the
         * text of our [TextView] field [mStatus] to "BOUND". If it is not checked and we
        ...
        private val mConnection: ServiceConnection = object : ServiceConnection {
             ...
             *
             * We initialize our [IRemoteService] field [mService] from
             * the [IBinder] parameter [service] passed us, If our [mServiceBound] flag is
             * *true* we set the text of our [TextView] field [mStatus] to "CONNECTED".
             *
             ...
             *
             * We set our field [IRemoteService] field [mService] to *null*, and if our flag
             * [mServiceBound] is *true* we set the text of our [TextView] field [mStatus]
             * to "DISCONNECTED".
             *
        ...
        /**
         * This constructor initializes fields needed to start the service with the `Class` given
         * by our `mClz` field, sets the `OnClickListener` of Buttons used to control the service,
         * and saves a reference to the `TextView` to use to display the status of the service.
         *
         * We locate the `Button` with resource ID `start` and set its `OnClickListener` to the
         * `OnClickListener mStartListener` field, locate the `Button` with resource ID `stop`
         * and set its `OnClickListener` to the `OnClickListener mStopListener` field, and locate
         * the `CheckBox` with resource ID `bind` and set its `OnClickListener` to the
         * `OnClickListener mBindListener` field. Finally we initialize our `TextView` field `mStatus`
     ...
     * our [ServiceInfo] fields [mService1], and [mService2]. The constructor not only initializes
     * fields, it also sets the `OnClickListener` for the various widgets in our UI based on the
     * resource IDs passed it.
     *
     ...
     * the services referenced by [ServiceInfo] field [mService1] and [ServiceInfo] field
     * [mService2] to unbind from their service if they are currently bound to it.
     */
