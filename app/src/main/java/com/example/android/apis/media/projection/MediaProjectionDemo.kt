...
class MediaProjectionDemo : AppCompatActivity() {
     ...
     * to our field `MediaProjection mMediaProjection` from the intent data that is received by
    ...
    /**
     * The `Surface` that provides direct access to the surface object underlying our field
    ...
    /**
     * `ToggleButton` with id R.id.screen_sharing_toggle in our layout file, its on click
     * callback is set with the attribute android:onClick="onToggleScreenShare". `onToggleScreenShare`
     ...
     * `onCreate`, then we set our content view to our layout file R.layout.media_projection.
     * We create a new instance for `DisplayMetrics metrics`, then retrieve the window manager
     ...
     * display into `DisplayMetrics metrics`. We then initialize our field `int mScreenDensity`
     * with the screen density field in `metrics`, expressed as dots-per-inch (may be either
     * DENSITY_LOW, DENSITY_MEDIUM, or DENSITY_HIGH).
     *
     *
     * We initialize our field `SurfaceView mSurfaceView` by locating the `SurfaceView`
     * in our layout file with ID R.id.surface. We fetch the SurfaceHolder providing access and
     * control of the underlying surface of `SurfaceView mSurfaceView`, then set our field
     * `Surface mSurface` to its `Surface`. Next we initialize our field
     * `MediaProjectionManager mProjectionManager` with the handle to the system-level service
     * MEDIA_PROJECTION_SERVICE (used for managing media projection sessions).
     *
     *
     * We create `ArrayAdapter<Resolution> arrayAdapter` from our list `List<Resolution> RESOLUTIONS`
     * using android.R.layout.simple_list_item_1 as the layout file containing a TextView to use when
     * instantiating views. We locate the `Spinner` with ID R.id.spinner in our layout file in
     * order to set `Spinner s`, set its adapter to `arrayAdapter`, set its `OnItemSelectedListener`
     * to a new instance of our class `ResolutionSelector`, and set its selection to 0.
     *
     *
     * Finally we locate the `ToggleButton` with ID R.id.screen_sharing_toggle in our layout
     * file in order to initialize our field `ToggleButton mToggle`, and call its method
     ...
     * implementation of `onDestroy`, then if our field `MediaProjection mMediaProjection`
     ...
     * `requestCode` returned is PERMISSION_CODE, and if not we log the problem and return
     * having done nothing. If the `resultCode` is not RESULT_OK, we toast the message "User
     * denied screen sharing permission" and return having done nothing.
     *
     *
     * If everything is correct, we initialize our field `MediaProjection mMediaProjection` to
     * the MediaProjection obtained from the successful screen capture request contained in the returned
     * `Intent data`. We register a new instance of our class `MediaProjectionCallback`
     * as the callback for `MediaProjection mMediaProjection`, and finally initialize our field
     * `VirtualDisplay mVirtualDisplay` with the instance of `VirtualDisplay` returned by
     * our method `createVirtualDisplay`.
     *
     ...
     * of the `View view` is "checked" we call our method `shareScreen`, otherwise we call
     * our method `stopScreenSharing`.
     *
    ...
    /**
     * Starts sharing the screen. First we set our flag `boolean mScreenSharing` to true, then
     * if our field `Surface mSurface` is null we return without doing anything more. If our
     * field `MediaProjection mMediaProjection` is null we start an activity for its result
     * using the intent to start screen capture created by our `MediaProjectionManager mProjectionManager`
     * and using PERMISSION_CODE as the request code, and return. Otherwise we set our field
     * `VirtualDisplay mVirtualDisplay` to the `VirtualDisplay` returned by our method
    ...
    /**
     * Stops screen sharing. If `ToggleButton mToggle` is "checked", we set it to unchecked,
     * then we set our flag `mScreenSharing` to false. If our field `VirtualDisplay mVirtualDisplay`
     * is not null, we release the virtual display and destroy its underlying surface, and set
     * `mVirtualDisplay` to null.
     */
     ...
     * `Surface mSurface`. We return the `VirtualDisplay` returned from the
     * `createVirtualDisplay` method of our field `MediaProjection mMediaProjection`.
     * The parameters passed to its `createVirtualDisplay` are:
     *
     *  *
     ...
     *
     *
     * @return a `VirtualDisplay` drawing the screen capture to `Surface mSurface`.
     */
    ...
    /**
     * Re-sizes our `VirtualDisplay mVirtualDisplay` to be `mDisplayWidth` by `mDisplayHeight`
     * with density `mScreenDensity` if `mVirtualDisplay` is not null. Called only from
     * the `surfaceChanged` method of our class `SurfaceCallbacks` which is UNUSED, so this
     * is unused as well.
     */
    ...
    /**
     * `OnItemSelectedListener` of the `Spinner` with ID R.id.spinner, it allows the
     * user to select a `Resolution` from the list `List<Resolution> RESOLUTIONS`.
     */
    private inner class ResolutionSelector : OnItemSelectedListener {
         ...
         * retrieve the item that has been selected to `Resolution r`. We fetch the layout
         * parameters of `SurfaceView mSurfaceView` (the view our virtual display is drawing
         * to) into `ViewGroup.LayoutParams lp`. We fetch our packages resources current
         * configuration in order to check the current orientation. If the orientation is ORIENTATION_LANDSCAPE
         * we set `mDisplayHeight` to `r.y` and `mDisplayWidth` to `r.x`, otherwise
         * we set `mDisplayHeight` to `r.x` and `mDisplayWidth` to `r.y`.
         * We set the `height` field of `lp` to `mDisplayHeight` and the `width`
         * field to `mDisplayWidth`, and finally set the layout parameters of `mSurfaceView`
         * to the modified `lp`.
         *
    ...
    private inner class MediaProjectionCallback : MediaProjection.Callback() {
         ...
         * to null and call our method `stopScreenSharing` to release the virtual display if
         * it exists.
         */
        override fun onStop() {
            mMediaProjection = null
    ...
    private inner class SurfaceCallbacks : SurfaceHolder.Callback {
         ...
         *
         *
         * We store the new `width` in `mDisplayWidth`, the new `height` in
         * `mDisplayHeight` and call our method `resizeVirtualDisplay` to resize our
         * virtual display.
         *
         ...
         * `Surface mSurface` with direct access to the surface object of `holder`. Then
         * if our flag `boolean mScreenSharing` is true, we call our method `shareScreen`
         ...
         * `boolean mScreenSharing` is not true we call our method `stopScreenSharing`
